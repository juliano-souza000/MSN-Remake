...
public class ListenForUsernameUpdate extends Thread
{
    private static SocketChannel server;
    private MemberUpdated updateListener;
    private boolean isAlive = true;
    
    public ListenForUsernameUpdate(MemberUpdated listener)
    {
	...
	if(server != null)
	{
	    try
	    {
		NetworkUtils.startListeningForUpdates(server, UpdateType.UsernameUpdated);
	    }
	    catch (IOException ex)
	    {
	        Logger.getLogger(getClass().getName()).log(Level.SEVERE, null, ex);
		...
		try
		{
			...
			if(response.has("Username"))
			    updateListener.UpdateUsername(response);
		    }
		}
		catch (IOException ex)
		{
