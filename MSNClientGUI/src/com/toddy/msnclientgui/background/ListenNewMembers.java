...
public class ListenNewMembers extends Thread
{
    private static SocketChannel server;
    private MemberUpdated updateListener;
    private boolean isAlive = true;
    
    public ListenNewMembers(MemberUpdated listener)
    {
	...
	if(server != null)
	{
	    try
	    {
		NetworkUtils.startListeningForUpdates(server, UpdateType.MemberAdded);
	    }
	    catch (IOException ex)
	    {
	        Logger.getLogger(ListenNewMembers.class.getName()).log(Level.SEVERE, null, ex);
		...
		try
		{
			...
			if(response.has("Profile"))
			    updateListener.UpdateNewMember(response.getJSONObject("Profile"));
		    }
		}
		catch (IOException ex)
		{
