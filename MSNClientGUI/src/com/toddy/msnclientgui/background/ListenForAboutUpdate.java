...
public class ListenForAboutUpdate extends Thread
{
    private static SocketChannel server;
    private MemberUpdated updateListener;
    private boolean isAlive = true;
    
    public ListenForAboutUpdate(MemberUpdated listener)
    {
	...
	if(server != null)
	{
	    try
	    {
		NetworkUtils.startListeningForUpdates(server, UpdateType.AboutUpdated);
	    }
	    catch (IOException ex)
	    {
	        Logger.getLogger(getClass().getName()).log(Level.SEVERE, null, ex);
		...
		try
		{
			...
			if(response.has("About"))
			    updateListener.UpdateAbout(response);
		    }
		}
		catch (IOException ex)
		{
